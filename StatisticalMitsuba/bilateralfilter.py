    ...
    if I.ndim == 2 and T.ndim == 2:  # I1T1 filter
        for y in range(r, r + h):
            for x in range(r, r + w):
                wgt = (
                    LUT[np.abs(T[y - r : y + r + 1, x - r : x + r + 1] - T[y, x])]
                    * kernel_s
                )
                output[y - r, x - r] = np.sum(
                    wgt * I[y - r : y + r + 1, x - r : x + r + 1]
                ) / np.sum(wgt)
    elif I.ndim == 3 and T.ndim == 2:  # I3T1 filter
        for y in range(r, r + h):
            for x in range(r, r + w):
                wgt = (
                    LUT[abs(T[y - r : y + r + 1, x - r : x + r + 1] - T[y, x])]
                    * kernel_s
                )
                wacc = np.sum(wgt)
                output[y - r, x - r, 0] = (
                    np.sum(wgt * I[y - r : y + r + 1, x - r : x + r + 1, 0]) / wacc
                )
                output[y - r, x - r, 1] = (
                    np.sum(wgt * I[y - r : y + r + 1, x - r : x + r + 1, 1]) / wacc
                )
                output[y - r, x - r, 2] = (
                    np.sum(wgt * I[y - r : y + r + 1, x - r : x + r + 1, 2]) / wacc
                )
    elif I.ndim == 3 and T.ndim == 3:  # I3T3 filter
        for y in range(r, r + h):
            for x in range(r, r + w):
                wgt = (
                    LUT[abs(T[y - r : y + r + 1, x - r : x + r + 1, 0] - T[y, x, 0])]
