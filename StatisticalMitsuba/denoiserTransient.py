...
class StatDenoiser(nn.Module):
    ...
    def compute_membership(self, estimands, estimands_variance, spp):
        gamma_w = self.compute_gamma_w(spp, spp).to(estimands.device)
        center_idx = self.n_patches // 2
...
if __name__ == "__main__":
    ...
    original_np = image.squeeze(0).permute(1, 2, 0).cpu().numpy().astype(np.float32)
